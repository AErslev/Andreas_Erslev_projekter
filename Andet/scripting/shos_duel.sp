...
{  
	...
	SetCookieMenuItem(DuelCookieHandler, 0, "SHOS Duel");
...
public Action msg(Handle timer, any client)
{
	...
	{
		CPrintToChat(client, "{green}[SHOS Duel]\x01 %t", "command");
	}
}
...
public Action DuelMenu(int client, int args)
{
	...
	SetMenuTitle(g_DuelMenu, "SHOS Duel Settings");
	switch(cookievalue)
	{
		case 0:
		{
...
void RefreshSounds(int client)
{
	...
	if(size > 0)
		CReplyToCommand(client, "{green}[SHOS Duel] {default}Loaded %d sounds.", size);
	else
		CReplyToCommand(client, "{green}[SHOS Duel] {default}INVALID SOUND PATH");
}
...
public void VoteYes(int client)
{
	...
	{
		CPrintToChatAll("{green}[SHOS Duel] \x01%t", "Duel Accepted Red", nome);
	}
	else if(GetClientTeam(client) == 3)
	{
		CPrintToChatAll("{green}[SHOS Duel] \x01%t", "Duel Accepted Blue", nome);
	}
	check_votes();
}
...
public void VoteNo(int client)
{
	...
	{
		case 2: {
			CPrintToChatAll("{green}[SHOS Duel] \x01%t", "Duel Refused Red", nome);
		}
		case 3: {
			CPrintToChatAll("{green}[SHOS Duel] \x01%t", "Duel Refused Blue", nome);
		}
	}
...
public void check_votes()
{
	if (g_S == 2)
	{
		...
		CPrintToChatAll("{green}[SHOS Duel] \x01%t", "Duel Canceled");
}
...
{   
	...
	{
		NoScopeEnabled = false;
		CPrintToChatAll("{green}[SHOS Duel] \x01%t", "Scope On");
		for (int  i = 1; i <= MaxClients; i++)
		{
	...
	{
		NoScopeEnabled = true;
		CPrintToChatAll("{green}[SHOS Duel] \x01%t", "Scope Off");
		return Plugin_Handled;
	}
}
...
public void StartDuel()
{
	...
	CPrintToChatAll("{green}[SHOS Duel] {default}%t", "Start Duel");
}
