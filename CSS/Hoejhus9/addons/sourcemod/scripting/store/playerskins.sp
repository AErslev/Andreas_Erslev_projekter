...
public PlayerSkins_Config(&Handle:kv, itemid)
{
	...
	if(FileExists(g_ePlayerSkins[g_iPlayerSkins][szModel]) || FileExists(g_ePlayerSkins[g_iPlayerSkins][szModel], true))
	{
		++g_iPlayerSkins;
		return true;
	}
...
public Action:PlayerSkins_PlayerSpawn(Handle:event,const String:name[],bool:dontBroadcast)
{
	...
	CreateTimer(0.2, PlayerSkins_PlayerSpawnPost, GetClientUserId(client));
...
public Store_SetClientModel(client, const String:model[])
{
	if(GAME_TF2)
		{
