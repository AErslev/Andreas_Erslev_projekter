...
public OnPluginStart()
{
	...
	g_cvarGiftEnabled = RegisterConVar("sm_store_enable_gifting", "1", "Enable/disable gifting of already bought items.", TYPE_INT);
...
{
	SQL_TQuery(g_hDatabase, SQLCallback_Void, "DROP TABLE store_players");
	SQL_TQuery(g_hDatabase, SQLCallback_Void, "DROP TABLE store_items");
	SQL_TQuery(g_hDatabase, SQLCallback_Void, "DROP TABLE store_equipment");
...
public AdminMenu_ResetPlayer(Handle:topmenu, TopMenuAction:action, TopMenuObject:object_id, client, String:buffer[], maxlength)
{
	...
	else if (action == TopMenuAction_SelectOption)
	{
		...
		LoopAuthorizedPlayers(i)
		{
			...
			GetClientAuthString(i, STRING(m_szAuthId));
...
public Native_GiveItem(Handle:plugin, numParams)
{
	...
	new m_iDateOfPurchase = GetTime();
	new m_iDateOfExpiration = 0;
	...
	if(parent != -1)
	{
		if(g_eItems[parent][iPrice]>0)
		{
			if(!Store_IsClientVIP(target) && !Store_IsItemInBoughtPackage(target, parent))
			{
				...
				if(g_eCvars[g_cvarGiftEnabled][aCache])
				{
		...
		if(g_eItems[i][iParent]==parent && (g_eCvars[g_cvarShowVIP][aCache] == 0 && GetClientPrivilege(target, g_eItems[i][iFlagBits], m_iFlags) || g_eCvars[g_cvarShowVIP][aCache]))
		{
			...
			if(g_eItems[i][iHandler] == g_iPackageHandler)
			{
				...
				if(g_eItems[i][iPrice] == -1 || Store_HasClientItem(target, i))
					AddMenuItem(m_hMenu, m_szId, g_eItems[i][szName]);
				else if(!g_bInvMode[client] && g_eItems[i][iPlans]==0)
			...
			else
			{
				...
				{				
					...
					if(g_eClients[target][iCredits]<m_iPrice || (g_eCvars[g_cvarShowVIP][aCache] && !GetClientPrivilege(target, g_eItems[i][iFlagBits], m_iFlags)))
...
public SQLCallback_Connect(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	...
	else
	{
		...
		{
			g_bMySQL = true;
			SQL_TQuery(g_hDatabase, SQLCallback_Void, "CREATE TABLE IF NOT EXISTS `store_players` (\
			...
			SQL_TQuery(g_hDatabase, SQLCallback_Void, "CREATE TABLE IF NOT EXISTS `store_items` (\
										  `id` int(11) NOT NULL AUTO_INCREMENT,\
			...
			SQL_TQuery(g_hDatabase, SQLCallback_Void, "CREATE TABLE IF NOT EXISTS `store_equipment` (\
										  `player_id` int(11) NOT NULL,\
										  `type` varchar(16) NOT NULL,\
										  `unique_id` varchar(256) NOT NULL,\
										  `slot` int(11) NOT NULL\
										)");
			SQL_TQuery(g_hDatabase, SQLCallback_Void, "CREATE TABLE IF NOT EXISTS `store_logs` (\
										  `id` int(11) NOT NULL AUTO_INCREMENT,\
										  `player_id` int(11) NOT NULL,\
										  `credits` int(11) NOT NULL,\
...
public Store_SaveClientData(client)
{
	...
	decl String:m_szQuery[256];
	Format(STRING(m_szQuery), "UPDATE store_players SET `credits`=%d, `date_of_last_join`=%d, `name`=\"%s\" WHERE `id`=%d", g_eClients[client][iCredits], g_eClients[client][iDateOfLastJoin], g_eClients[client][szNameEscaped], g_eClients[client][iId]);
	SQL_TQuery(g_hDatabase, SQLCallback_Void, m_szQuery);
}
...
Store_BuyItem(client, itemid, plan=-1)
{
	...
	g_eClientItems[client][m_iId][iDateOfExpiration] = (plan==-1?0:GetTime()+g_ePlans[itemid][plan][iTime]);
...
Store_PackageHasClientItem(client, packageid, bool:invmode=false)
{
	...
	if(!GetClientPrivilege(client, g_eItems[packageid][iFlagBits], m_iFlags))
		return false;
	for(new i=0;i<g_iItems;++i)
