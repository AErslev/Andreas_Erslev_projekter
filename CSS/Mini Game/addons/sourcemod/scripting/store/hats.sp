...
#endif
{
	...
	HookEvent("player_death", Hats_PlayerDeath);
}
...
public Hats_OnMapStart()
{
	...
	// Just in case...
	if(FileExists(g_eCvars[g_cvarDefaultT][sCache]) || FileExists(g_eCvars[g_cvarDefaultT][sCache], true))
	{
		g_bTOverride = true;
	...
	if(FileExists(g_eCvars[g_cvarDefaultCT][sCache]) || FileExists(g_eCvars[g_cvarDefaultCT][sCache], true))
	{
		g_bCTOverride = true;
...
public Hats_Config(&Handle:kv, itemid)
{
	...
	if(!(FileExists(g_eHats[g_iHats][szModel]) || FileExists(g_eHats[g_iHats][szModel], true)))
		return false;
...
public Action:Hats_PlayerDeath(Handle:event, const String:name[], bool:dontBroadcast)
{
	new client = GetClientOfUserId(GetEventInt(event, "userid"));
	for(new i=0;i<STORE_MAX_SLOTS;++i)
		RemoveHat(client, i);
