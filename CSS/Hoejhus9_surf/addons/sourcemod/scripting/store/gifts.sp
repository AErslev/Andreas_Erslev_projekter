...
public Gifts_MenuHandler(Handle:menu, MenuAction:action, client, param2)
{
	if(action == MenuAction_Select)
	{
		if(menu == INVALID_HANDLE)
		{
			...
			pos[2]+=20.0;
			Gifts_SpawnGift(Gifts_OnPickUpItem, "", -1.0, pos, param2, GetClientUserId(target));
...
{
	new owner_id = GetClientOfUserId(owner);
	if(!owner_id || !IsClientInGame(owner_id))
		return;

	new m_eItem[Store_Item];
	new m_eHandler[Type_Handler];
	Store_GetItem(data, m_eItem);
	Store_GetHandler(m_eItem[iHandler], m_eHandler);

	Store_GiveItem(client, data);
