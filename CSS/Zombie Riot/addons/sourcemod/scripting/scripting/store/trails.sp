...
public Trails_Config(&Handle:kv, itemid)
{
	...
	if(FileExists(g_eTrails[g_iTrails][szMaterial]) || FileExists(g_eTrails[g_iTrails][szMaterial], true))
	{
		++g_iTrails;
		return true;
	}
	...
	if(m_iEquipped >= 0)
	{
		...
		if(GAME_CSGO)
		{
			if(g_iClientTrails[client][slot] == 0 || !IsValidEdict(g_iClientTrails[client][slot]))
			{
				...
				DispatchKeyValue(g_iClientTrails[client][slot], "model", g_eTrails[m_iData][szModel]);
...
public AttachTrail(ent, client, current, num)
{
	...
	new Float:m_fX = (Float:g_eCvars[g_cvarPadding][aCache]*((num%g_eCvars[g_cvarMaxColumns][aCache])-1))/2-(Float:g_eCvars[g_cvarPadding][aCache]*(current%g_eCvars[g_cvarMaxColumns][aCache]));
