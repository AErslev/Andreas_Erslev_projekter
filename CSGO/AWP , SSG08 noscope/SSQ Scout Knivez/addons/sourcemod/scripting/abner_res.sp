...
public void Event_RoundEnd(Handle event, const char[] name, bool dontBroadcast)
{
	...
	if((winner == CS_TEAM_CT && SamePath) || winner == CS_TEAM_T)
	{
		...
		else
		{
			if(!SamePath) 
			{
				PrintToServer("[RES] TR_SOUNDS ERROR: Sounds not loaded.");
				CPrintToChatAll("{green}[RES] {default}TR_SOUNDS ERROR: Sounds not loaded.");
			}
			else
			{
				PrintToServer("[RES] SOUNDS ERROR: Sounds not loaded.");
				CPrintToChatAll("{green}[RES] {default}SOUNDS ERROR: Sounds not loaded.");
			}
			return;
		}
	}
	else if(winner == CS_TEAM_CT)
	{
		...
		{
			PrintToServer("[RES] CT_SOUNDS ERROR: Sounds not loaded.");
			CPrintToChatAll("{green}[RES] {default}CT_SOUNDS ERROR: Sounds not loaded.");
			return;
		}
	}
...
public Action msg(Handle timer, any client)
{
	...
	{
		CPrintToChat(client, "{default}{green}[RES] {default}%t", "JoinMsg");
	}
}
...
void RefreshSounds(int client)
{
	...
	if(SamePath)
	{
		...
		if(SoundsTRSucess)
		{
			ReplyToCommand(client, "[RES] SOUNDS: %d sounds loaded.", size);
		}
		else
		{
			ReplyToCommand(client, "[RES] INVALID SOUND PATH.");
		}
	}
	else
	{
		...
		if(SoundsTRSucess)
		{
			ReplyToCommand(client, "[RES] TR_SOUNDS: %d sounds loaded.", size);
		}
		else
		{
			ReplyToCommand(client, "[RES] INVALID TR SOUND PATH.");
		}
		...
		if(SoundsCTSucess)
		{
			ReplyToCommand(client, "[RES] CT_SOUNDS: %d sounds loaded.", size);
		}
		else
		{
			ReplyToCommand(client, "[RES] INVALID CT SOUND PATH.");
		}
	}
}
...
void PlayMusicAll(char[] szSound)
{
	...
	{
		CPrintToChatAll("{green}[RES] {default}%t", "mp3 print", szSound);
	}
}
