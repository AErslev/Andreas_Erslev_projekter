...
public OnMapStart() {
	...
	PrecacheSoundAny("hssound/impact.mp3", true);
}
public OnPluginStart() {
	CreateConVar("csgo_hs_impact", PLUGIN_VERSION, "[CSGO] Hs Impact Effect", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY);
...
public PlayerDeath(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (GetEventBool(event, "headshot") && IsValidClient(iClient) && (g_bHS_enabled) && (g_bClientPrefHsOverlays[iClient])){
		switch (GetRandomInt(0, 1))
		{
			case 0:
			{
				EmitSoundToClientAny(iClient, "hssound/impact.mp3");
