...
public void OnPluginStart()
{
	...
	g_hDBConfig = AutoExecConfig_CreateConVar("group_dbconfig", "group", "database config (MySQL) to use for this plugin (not working correctly)");
	g_hGroupId = AutoExecConfig_CreateConVar("group_groupid", "103582791436057506", "Your Group ID in the format as default (not working correctly)");
	...
	char error[256];
	g_DB = SQL_Connect("group", true, error, sizeof(error));
...
public Action cmdClaimCredits(int client, int args) {
	SteamWorks_GetUserGroupStatus(client, 103582791436057506);
...
public void sql_alreadyGottenCreditsQuery(Handle owner, Handle hndl, const char[] error, any client) {
	if (hndl != INVALID_HANDLE) {
		while (SQL_FetchRow(hndl)) {
			...
			CPrintToChat(client, "{green}[{purple}%s{green}]{lightred} You have already recieved {green}%i Credits{lightred} At the: {green}%s{lightred}.", g_cTag, amount, timestamp);
		}
	}
}
...
public void sql_CheckGroupCallback(Handle owner, Handle hndl, const char[] error, any userid)
{
	if (hndl != INVALID_HANDLE)
	{
		...
		CPrintToChat(client, "{green}[{purple}%s{green}]{orange} You have recieved {green}%i{orange} Credits because you joined our Steam Group!", g_cTag, g_iCreditsAmount);
		char query[255];
		char c_id[20];
