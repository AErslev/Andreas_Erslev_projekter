...
new Handle:cvar_throwingknives_dev;
...
public Plugin:myinfo = {
	...
	author = "original by meng, Bacardi, Bara",
	version = "2016-1-11",
...
public OnPluginStart()
{
	...
	cvar_throwingknives_dev = CreateConVar("sm_throwingknives_dev", "0", "Enable/disable dev. mode.", _, true, 0.0, true, 1.0);
...
public Action:player_death(Handle:event,const String:name[],bool:dontBroadcast)
{
	...
	if(StrContains(weapon, "knife", false) != -1) // All knive kills, because csgo
	{
		...
		if(GetConVarBool(cvar_throwingknives_steal) && GetConVarInt(cvar_throwingknives_count) > 0)
		{
			...
			{
				if (GetConVarBool(cvar_throwingknives_dev))
				{
					Call_StartForward(g_hKKForward);
					Call_PushCell(victim);
					Call_PushCell(attacker);
					Call_PushCell(g_bHeadshot[attacker]);
					Call_Finish();
				}
...
public NativeSetClientThrowingKnives(Handle:plugin, numParams)
{
	...
	g_iPlayerKniveCount[client] = num;
	
	if(g_iPlayerKniveCount[client] > GetConVarInt(cvar_throwingknives_count))
		g_iPlayerKniveCount[client] = GetConVarInt(cvar_throwingknives_count);
