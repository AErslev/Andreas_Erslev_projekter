...
int g_testcase_runned;


char curl_test_path[512];
...
static char ret_sha1[][] = {
	"a9993e364706816aba3e25717850c26c9cd0d89d",
	"84983e441c3bd26ebaae4aa1f95129e5e54670f1"
};


static char test_sha224_to_512[][] = {
	"",
	"a",
	"abc",
	"message digest",
...
Handle test_3_file = INVALID_HANDLE;

Handle test_5_file_body = INVALID_HANDLE;
Handle test_5_file_header = INVALID_HANDLE;

Handle test_6_file = INVALID_HANDLE;

Handle test_7_form = INVALID_HANDLE;
Handle test_7_file = INVALID_HANDLE;

Handle test_8_file = INVALID_HANDLE;
...
public void PluginInit()
{
	...
	Handle test_folder_handle = OpenDirectory(curl_test_path);
...
public void onComplete(Handle hndl, CURLcode code, any data)
{
	int current_test = data;
...
public Action curl_self_test(int client, int args)
{
	if(g_test_running)
	{
...
void ExecCURL(Handle curl, int current_test)
{
#if USE_THREAD
	curl_easy_perform_thread(curl, onComplete, current_test);
#else
	CURLcode code = curl_load_opt(curl);
	if(code != CURLE_OK) {
...
public void TestCaseEndPreClose(int current_test)
{

}

public void TestCaseEndPostClose(int current_test)
{
	switch(current_test)
	{
		case 3:
...
public void Test_7_Action()
{
	Handle regex = CompileRegex(TEST_7_REGEX);
	...
	Handle file = OpenFile(file_path,"r");
	...
	{
		RegexError ret;
		int pos = MatchRegex(regex, buffer, ret);
...
public Action curl_hash_test(int client, int args)
{
	for(int i=0;i<sizeof(test_md5);i++)
	{
	...
	for(int i=0;i<sizeof(test_md4);i++)
	{
	...
	for(int i=0;i<sizeof(test_sha);i++)
	{
	...
	for(int i=0;i<sizeof(test_sha1);i++)
	{
	...
	for(int i=0;i<sizeof(test_sha224_to_512);i++)
	{
	...
	for(int i=0;i<sizeof(test_ripemd160);i++)
	{
...
public void hash_complete_callback(const bool success, const char []buffer)
{
	if(!success)
	{
...
public bool CheckHash(const char []data, Openssl_Hash type, const char []ret)
{
	static char hash_type[][16] = {
		"md5",
		"md4",
		"sha",
		"sha1",
...
public Handle CreateTestFile(const char []path, const char []mode)
{
	char file_path[512];
...
void PrintCreatecUrlError(int index)
{
	PrintToServer("[CURL Test #%d] curl_easy_init Fail",index);
	g_test_running = false;
}

void PrintTestCaseDebug(int index, const char []format, any ...)
{
	char buffer[256];
...
void PrintTestMessage(const char []format, any ...)
{
	char buffer[256];
...
void PrintHashTestDebug(const char []format, any ...)
{
	char buffer[256];
	VFormat(buffer, sizeof(buffer), format, 2);
